            ...
            <div class="row justify-content-center mb-3">
                <div class="col-md-8">
                    ...
                    <div class="input-group mb-3">
                        <input id="input_search" class="form-control" name="key" placeholder="Nhập tên màu sắc ..." />
    ...
    <div class="card mt-4 custom-border">
        <div class="card-body">
            <div class="d-flex justify-content-between mb-3">
                ...
                <a href="/admin/color/create" class="btn btn-teal"><i class="fa-solid fa-plus"></i> Thêm màu sắc</a>
            </div>
...
<script>
    $(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.data.length === 0) {
                        toastr.warning('Không tìm thấy màu sắc nào phù hợp.', 'Thông báo');
                    }
                    $.each(response.data, function (index, color) {
                        colorTable.row.add([
                            index + 1,
                            color.tenMauSac,
                            convertStatus(color.trangThai),
                            '<a href="/admin/color/detail/' + color.id + '" class="btn btn-info btn-sm mr-2"><i class="fa-solid fa-info"></i></a>' +
                            '<a href="/admin/color/update/' + color.id + '" class="btn btn-info btn-sm mr-2"><i class="fa-solid fa-pen"></i></a>'
                        ]);
                    });
                    colorTable.draw();
                },
                error: function () {
                    toastr.error('Đã xảy ra lỗi khi tải màu sắc.', 'Lỗi hệ thống');
                }
            });
        }
        ...
        $('input[name="status_type"]').change(loadTableColor);
        $('#btn_search').click(loadTableColor);
